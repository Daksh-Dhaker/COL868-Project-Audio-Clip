...
from zero_shot_eval import FLICKR_SPLIT_SEED
...
from zero_shot_eval import flickr_train_test_split
...
def run_generic_training(args: argparse.Namespace, q_type: str, r_type: str) -> None:
    ...
    # For Flickr8k: train on 70%, evaluate on 30%
    if args.dataset == "flickr8k":
        train_text, train_image, test_text, test_image = flickr_train_test_split(
            all_items["text"], all_items["image"]
        )
        train_items = {"text": train_text, "audio": [], "image": train_image}
        eval_items = {"text": test_text, "audio": [], "image": test_image}
        print(f"[Info] Flickr8k: train={len(train_image)} images, test={len(test_image)} images (seed={FLICKR_SPLIT_SEED})")
    else:
    ...
    pair_ds = PairDataset(
        query_items=train_items[q_type],
        result_items=train_items[r_type],
        seed=args.seed,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.eval_every > 0 and (epoch % args.eval_every == 0 or epoch == args.epochs):
            model.eval()
            with torch.no_grad():
                hit_by_k, r_by_k, sup_acc, nq, nr, _map_score, _sample_cls = evaluate_once_accuracy(
                    model,
                    q_type,
                    r_type,
                    eval_items,
