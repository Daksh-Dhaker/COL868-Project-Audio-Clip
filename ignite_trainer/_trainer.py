...
def run(experiment_name: str,
    ...
    with _utils.tqdm_stdout(orig_stdout) as orig_stdout:
        ...
        disable_checkpoints = os.environ.get('AUDIOCLIP_DISABLE_CHECKPOINTS', '').strip().lower() in {
            '1', 'true', 'yes', 'on'
        }

        if checkpoint_metrics and not disable_checkpoints:
            ...
            # Ignite API differs across versions; avoid passing unsupported kwargs.
            mc_params = inspect.signature(ihan.ModelCheckpoint.__init__).parameters
            if 'save_as_state_dict' in mc_params:
                saver_kwargs['save_as_state_dict'] = True
        ...
        elif checkpoint_metrics and disable_checkpoints:
            tqdm.tqdm.write('Checkpoint saving disabled (AUDIOCLIP_DISABLE_CHECKPOINTS=1).')
