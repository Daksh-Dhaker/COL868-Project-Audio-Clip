...
class _NoOpVisdom:
    ...
    def get_window_data(self, win: str, env: Optional[str] = None) -> Optional[str]:
        return None

    def win_exists(self, win: str, env: Optional[str] = None) -> bool:
        return False

    def line(self, X: np.ndarray, Y: np.ndarray, win: str, env: Optional[str] = None,
             update: str = 'append', name: Optional[str] = None, opts: Optional[dict] = None) -> str:
        return win

    def update_window_opts(self, win: str, opts: dict, env: Optional[str] = None) -> str:
        return win

    def text(self, text: str, win: str, env: Optional[str] = None,
             opts: Optional[dict] = None, append: bool = False) -> str:
        return win

    def save(self, envs: list[str]) -> None:
        return None


def calc_ytick_range(vis: Any, window_name: str, env: Optional[str] = None) -> Tuple[float, float]:
...
def create_summary_window(vis: Any,
                          visdom_env_name: str,
                          experiment_name: str,
                          summary: str) -> str:
...
def get_visdom_instance(host: str = 'localhost',
                        port: int = 8097,
                        env_name: str = 'main',
                        env_path: str = 'visdom_env') -> Tuple[Any, Optional[int]]:

    disable_visdom = os.environ.get('AUDIOCLIP_DISABLE_VISDOM', '').strip().lower() in {'1', 'true', 'yes', 'on'}
    if disable_visdom:
        tqdm.tqdm.write('Visdom disabled (AUDIOCLIP_DISABLE_VISDOM=1). Using console-only logging.')
        return _NoOpVisdom(), None

    if visdom is None:
        tqdm.tqdm.write('Visdom package not installed. Using console-only logging.')
        return _NoOpVisdom(), None
